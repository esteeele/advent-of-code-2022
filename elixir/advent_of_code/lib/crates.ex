  ...
  def solve do
    ...
    initial_state =
      ...
      |> Enum.map(fn line ->
        ...
        # i.e. get 2nd in e.g. ["[", "Z", "]", " "]
        |> Enum.map(fn lst -> hd(tl(lst)) end)
        |> List.foldr([], fn crate, acc -> [crate | acc] end)
      end)
      |> List.foldl(%{}, fn row, crates_map ->
        updated_map =
          List.foldl(Enum.to_list(1..length(row)), crates_map, fn index, crates_map ->
            new_val = Enum.at(row, index - 1)

            case Map.has_key?(crates_map, index) do
              true ->
                Map.update!(crates_map, index, fn existing_items ->
                  case new_val do
                    " " -> existing_items
    ...
    moved_crates =
      List.foldl(moves, initial_state, fn line, crates ->
